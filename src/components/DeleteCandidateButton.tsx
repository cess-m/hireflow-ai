'use client'

import { useState } from 'react'
import { useRouter } from 'next/navigation'
import { createClient } from '@/lib/supabase'

export default function DeleteCandidateButton({
  candidateId,
  candidateName,
  isShortlisted,
}: {
  candidateId: string
  candidateName: string
  isShortlisted: boolean
}) {
  const [state, setState] = useState<'idle' | 'confirm' | 'deleting'>('idle')
  const router = useRouter()

  const handleDelete = async () => {
    setState('deleting')
    const supabase = createClient()
    await supabase.from('candidates').delete().eq('id', candidateId)
    router.refresh()
  }

  if (state === 'idle') {
    return (
      <button
        onClick={() => setState('confirm')}
        title={`Delete ${candidateName}`}
        className="text-slate-300 hover:text-red-500 transition-colors duration-150 cursor-pointer"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
          <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
        </svg>
      </button>
    )
  }

  if (state === 'deleting') {
    return <span className="text-xs text-slate-400">Deleting...</span>
  }

  return (
    <div className="flex flex-col items-end gap-1.5">
      {isShortlisted && (
        <p className="text-xs text-amber-600 max-w-[180px] text-right leading-snug">
          This candidate was sent to Google Sheets and will remain there after deletion.
        </p>
      )}
      <div className="flex items-center gap-2">
        <button
          onClick={() => setState('idle')}
          className="text-xs text-slate-500 hover:text-slate-700 cursor-pointer"
        >
          Cancel
        </button>
        <button
          onClick={handleDelete}
          className="text-xs bg-red-500 hover:bg-red-600 text-white font-medium px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
        >
          Delete
        </button>
      </div>
    </div>
  )
}
